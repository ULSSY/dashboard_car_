...
def run_eda_app() :
    ...
    st.subheader('고객 정보 화면')
   
        
    df = pd.read_csv('data/Car_Purchasing_Data.csv', encoding='ISO-8859-1')
    st.sidebar.header('<목록>')
    st.sidebar.subheader('1)고객 정보 데이터프레임')
    st.sidebar.subheader('2)고객 정보 통계치')  

    st.sidebar.subheader('3)상관계수 그래프')
    st.sidebar.subheader('4)고객정보 불러오기')
    
   
    
    st.dataframe(df)
    st.subheader('고객 정보 통계치')
    st.dataframe( df.describe() )
    df1=df.describe()
    # st.scatterplot(data=df1)
    st.subheader('상관계수 그래프 확인')
    df_corr=df.iloc[:,3:]
    ...
    print(df.columns)
    st.subheader('고객 정보 불러오기')
    selected_columns = st.multiselect('고객정보의 컬럼을 선택하세요', df.columns)
